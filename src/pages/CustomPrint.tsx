        ...
        <section className="py-16 bg-card border-b border-border">
          <div className="container mx-auto px-4 lg:px-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
              <h1 className="text-4xl md:text-5xl font-display font-bold text-foreground mb-4">Custom 3D Print</h1>
              <p className="text-muted-foreground text-lg">Upload your 3D model and get an instant quote. We support STL, OBJ, and ZIP files.</p>
            </motion.div>
          </div>
        </section>
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              ...
              <div
                ...
                onDrop={handleDrop}
              >
                <input type="file" accept=".stl,.obj,.zip" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              ...
              <div className="space-y-4 p-6 rounded-2xl bg-card border border-border">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {colors.map((c) => (
                      <button key={c.name} onClick={() => setSelectedColor(c.name)}
                        className={`w-8 h-8 rounded-full border-2 ${selectedColor === c.name ? 'border-primary scale-110' : 'border-border'}`}
                ...
                <div>
                  ...
                  <Slider value={infill} onValueChange={setInfill} min={10} max={100} step={5} />
                </div>

                <div>
                  <label className="text-sm font-medium mb-2 block">Quantity</label>
                  <Input type="number" min={1} value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value) || 1)} className="bg-secondary border-border" />
                </div>

                <div>
                  <label className="text-sm font-medium mb-2 block">Notes (optional)</label>
                  <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Special instructions..." className="bg-secondary border-border" />
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl bg-gradient-card border border-border">
                ...
                <Button size="lg" className="w-full mt-6" onClick={handleAddToCart}><ShoppingCart className="w-5 h-5" />Add to Cart</Button>
              </div>
            </motion.div>
          </div>
        </section>
